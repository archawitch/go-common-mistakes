d3 = (function () {
  ...
  var d3_event_dragSelect = d3_vendorSymbol(
      d3_documentElement.style,
      "userSelect"
    ),
    d3_event_dragId = 0;
  ...
  d3.behavior.zoom = function () {
    ...
    function mousedowned() {
      ...
      function ended() {
        w.on(mousemove, d3_window === target ? mousewheelreset : null).on(
          mouseup,
          null
        );
        dragRestore(dragged && d3.event.target === eventTarget);
  ...
  function d3_timer_mark() {
    ...
    while (d3_timer_active) {
      if (now >= d3_timer_active.time)
        d3_timer_active.flush = d3_timer_active.callback(
          now - d3_timer_active.time
        );
  ...
  function d3_geo_clipSort(a, b) {
    return (
      ((a = a.point)[0] < 0 ? a[1] - halfπ - ε : halfπ - a[1]) -
    ...
    for (var i = 0, n = polygon.length; i < n; ++i) {
      ...
      while (true) {
        ...
        d3_geo_areaRingSum.add(
          Math.atan2(k * Math.sin(dλ), cosφ0 * cosφ + k * Math.cos(dλ))
        );
  ...
  function d3_geo_clipExtent(x0, y0, x1, y1) {
    ...
    function clipLine(a, b) {
      ...
      if (
        d3_geo_clipExtentT(x0 - a[0], dx, t) &&
  ...
  function d3_scale_ordinal(domain, ranger) {
    ...
    function scale(x) {
      return range[
        ((index.get(x) ||
          (ranger.t === "range" && index.set(x, domain.push(x)))) -
          1) %
          range.length
      ];
    }
  ...
  function d3_time_parseWeekday(date, string, i) {
    ...
    return n
      ? ((date.w = d3_time_dayLookup.get(n[0].toLowerCase())), i + n[0].length)
      : -1;
  }
